...
export const types = {
  ...
  FETCH_RELATED_KEYWORDS_SUCCESS: 'SEARCH/FETCH_POPULAR_RELATED_SUCCESS',
  FETCH_RELATED_KEYWORDS_FAILURE: 'SEARCH/FETCH_POPULAR_RELATED_FAILURE',
  //设置当前输入
...
export const actions = {
  ...
  loadPopularKeywords: () => {
    return (disptch, getState) => {
      const { ids } = getState().search.popularKeyords;
      if(ids.length > 0) {
        return null;
      }
...
//reducer
const popularkeywords = (state = initialState.popularKeyords, action) => {
  switch (action.type) {
...
const historyKyewords = (state = initialState.historyKyewords, action) => {
  switch(action.type) {
...
export const getPopularKeywords = state => {
  return state.search.popularKeyords.ids.map(id => {
...
export const getRelatedKeywords = state => {
  ...
  return relatedKeywords.id.map(id => {
    return getKeywordById(state, id)
  })
}
